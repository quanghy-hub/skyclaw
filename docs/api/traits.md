...
SkyClaw defines 12 core traits in `skyclaw-core` (plus the `FileTransfer` sub-trait on `Channel`). Every subsystem is a trait implementation. Trait objects (`Box<dyn Trait>`) provide runtime polymorphism, with configuration determining which implementation is used.

All traits require `Send + Sync` and use `#[async_trait]` for async method support.

Source: `crates/skyclaw-core/src/traits/`
...
**Implementations**: `skyclaw-providers` crate -- `anthropic.rs`, `openai_compat.rs`, `google.rs`, `mistral.rs`, `groq.rs`
...
**Implementations**: `skyclaw-channels` crate -- `telegram.rs`, `discord.rs`, `slack.rs`, `whatsapp.rs`, `cli.rs`
...
**Implementations**: `skyclaw-channels/src/file_transfer.rs` -- per-channel implementations
...
**Implementations**: `skyclaw-tools` crate -- `shell.rs`, `file_ops.rs`, `browser.rs`, `git.rs`, `http.rs`, `screenshot.rs`
...
**Implementations**: `skyclaw-memory` crate -- `sqlite.rs`, `postgres.rs`, `markdown.rs`
...
**Implementations**: `skyclaw-vault` crate -- `local.rs` (ChaCha20-Poly1305), `resolver.rs` (vault:// URI resolution), `detector.rs` (API key pattern detection)
...
**Implementations**: `skyclaw-filestore` crate -- `local.rs`, `s3.rs`
...
**Implementations**: `skyclaw-observable` crate -- `logging.rs`, `metrics.rs`, `otel.rs`
